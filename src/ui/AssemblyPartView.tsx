...
export function AssemblyPartView({ set, pending }: { set: SliceSet | null; pending: boolean }) {
  ...
  const slice = set?.slices.find((s) => s.part?.id === id) ?? set?.slices[Math.min(Math.max(index - 1, 0), (set?.slices.length || 1) - 1)];
  ...
  return <section className="assembly-canvas assembly-part-view">
    <div className="assembly-overlay"><strong>Part</strong><select aria-label="Assembly part" value={slice?.part?.id ?? ''} onChange={(e) => { select(e.target.value); setIndex(set!.slices.find((s) => s.part?.id === e.target.value)!.index); }}>{set?.slices.map((s) => <option key={s.part?.id} value={s.part?.id}>{s.part?.label} · {s.part?.kind}</option>)}</select><span>{pending ? 'Rebuilding…' : 'Cut paths'}</span></div>
