...
import { alignCurvePoint, copyCurveLoops, copyCurveSketch, curveBounds, curveNodeMode, curveSketchError, ellipseCurve, moveCurveNode, resizeCurveLoops, splitCurve, straightenCurveSegment } from '../core/curves';
import { useKerros } from '../core/store';
import { NumberField } from './NumberField';
import { curveKeyLabel } from './curveSession';

type Tool = 'select' | 'pen' | 'line' | 'ellipse';
type Box = { x: number; y: number; w: number; h: number };
type Pick = { loop: number; node: number; part: 'point' | 'incoming' | 'outgoing' };
type Gesture = {
  kind: 'node' | 'pen' | 'line' | 'ellipse' | 'pan'; pointer: number; start: CurvePoint; client: CurvePoint;
...
export function CurveEditor({ feature, slices }: { feature: Feature; slices: SliceSet | null }) {
  ...
  useEffect(() => {
    if (owned.current !== sketch || ownerProject.current !== project) {
      ...
      setDraft(null); setPen(null); setHover(null); setSelected(null); setHistory({ undo: [], redo: [] }); setMessage('');
  ...
  const closePen = () => {
    ...
    const next = replaceLoop(tool === 'line' ? straightenCurveSegment(drawn, drawn.length - 1) : drawn); setDrawing(next);
    if (commit(next)) { setPenDrawing(null); setHover(null); setTool('select'); setSelected(null); }
  };
  const begin = (event: ReactPointerEvent, pick?: Pick) => {
    ...
    if (kind === 'pen' || kind === 'line') {
      ...
      const at = kind === 'line' && event.shiftKey && existing.length ? alignCurvePoint(existing.at(-1)!.point, start) : start;
      const next: CurveLoop = [...existing, { point: at, incoming: [0, 0], outgoing: [0, 0], smooth: false }];
      setPenDrawing(kind === 'line' && existing.length ? straightenCurveSegment(next, next.length - 2) : next); setHover(null);
    }
  ...
  const move = (event: ReactPointerEvent) => {
    ...
    if (!g) {
      const last = penRef.current?.at(-1);
      if (tool === 'line' && last) setHover(event.shiftKey ? alignCurvePoint(last.point, at) : at);
      return;
    }
    ...
    if (g.kind === 'line') return; // A line click never creates curve handles.
    if (g.kind === 'ellipse') {
  ...
  const end = (event: ReactPointerEvent) => {
    ...
    if (event.type === 'pointercancel' || useKerros.getState().projectRevision !== g.project) { setDrawing(null); setPenDrawing(null); setHover(null); return; }
  ...
  const changeTool = (value: Tool) => {
    const keepPath = (tool === 'pen' || tool === 'line') && (value === 'pen' || value === 'line');
    const cancelled = !!penRef.current && !keepPath;
    gesture.current = null; if (!keepPath) setPenDrawing(null);
    setDrawing(null); setHover(null); setTool(value); setMessage(cancelled ? 'Unfinished path cancelled.' : '');
  };
  ...
  return <div className="curve-editor" onKeyDown={event => {
    ...
    else if (event.key === 'Escape') { gesture.current = null; setPenDrawing(null); setDrawing(null); setHover(null); setTool('select'); setMessage('Edit cancelled.'); }
    else if (penRef.current) {
      if (['Enter', 'Delete', 'Backspace', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key)) { event.preventDefault(); event.stopPropagation(); }
      if (event.key === 'Enter') closePen();
    }
    ...
    <div className="curve-toolbar">
      ...
      <div className="curve-tool-row">
        {(['select', 'line', 'pen', 'ellipse'] as const).map(value => <button className={`btn${tool === value ? ' is-active' : ''}`} aria-pressed={tool === value} key={value} onClick={() => changeTool(value)}>{value === 'pen' ? 'Bézier pen' : value === 'line' ? 'Straight line' : value === 'ellipse' ? 'Ellipse / circle' : 'Select points'}</button>)}
    ...
    <div className="curve-canvas-wrap">
      <svg ref={svg} role="application" aria-label="Curve drawing canvas" tabIndex={0} viewBox={`${box.x} ${box.y} ${box.w} ${box.h}`}
        ...
        onLostPointerCapture={() => { if (gesture.current) { gesture.current = null; setDrawing(null); setPenDrawing(null); setHover(null); } }}>
        ...
        <g transform="scale(1 -1)">
          ...
          {pen && hover && tool === 'line' && <line x1={pen.at(-1)!.point[0]} y1={pen.at(-1)!.point[1]} x2={hover[0]} y2={hover[1]} stroke="#ffad69" strokeWidth={unit} strokeDasharray={`${5 * unit} ${4 * unit}`} pointerEvents="none" />}
        </g>
    ...
    <div className="curve-toolbar curve-bottom">
      ...
      <fieldset className="curve-dimensions" aria-label="Drawing dimensions" disabled={!!pen}>
      ...
      <div className="curve-help" role="status">{message || (tool === 'line' ? 'Click points for straight edges. Hold Shift for horizontal or vertical. Switch to Bézier pen to continue with curves. Close on the first point, with Enter or Close path. Esc cancels. Only closed shapes are applied.' : tool === 'pen' ? 'Click for corners; drag for smooth handles. Switch to Straight line to continue with straight edges. Close on the first point, with Enter or Close path. Esc cancels.' : tool === 'ellipse' ? 'Drag a bounding rectangle. Hold Shift for a circle. Replaces the selected loop unless Add loop / hole is checked.' : radial ? 'The right side of the axis generates the lamp; the shaded left side is retained but unused. Move Axis X in the inspector. Drag points or handles; right-drag pans. Grid: 10 mm.' : 'Drag points or handles. Arrow keys move a selected point 1 mm, Shift 5 mm. Right-drag pans. Other keys appear faintly. Grid: 10 mm. Drawing coordinates are local to this profile.')}</div>
    </div>
  </div>;
}
