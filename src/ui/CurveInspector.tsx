...
import { copyCurveLoops, copyCurveSketch, curveHeight, curveSketchError } from '../core/curves';
...
export function CurveInspector({ feature, slices, fresh }: { feature: Feature; slices: SliceSet | null; fresh: boolean }) {
  ...
  const sketch = feature.sketch!, morph = feature.params.curveMode === 'morph';
  ...
  return <>
    <div className="group">
      ...
      <button className="btn btn-wide" onClick={() => open ? close() : edit(feature.id, morph ? sketch.keys[0]?.id ?? null : null)}>{open ? 'View cut layers' : 'Edit curves in Slice'}</button>
